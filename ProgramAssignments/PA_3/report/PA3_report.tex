...
\begin{enumerate}
	\item \textbf{Initialization}:\\
        {\color{black}The root processor distributes input matrices $A$ and $B$ in global COO format to the $\sqrt{p} \times \sqrt{p}$ processor grid, with each processor receiving entries for its block without coordinate adjustments. }\\
    ...
    \end{itemize}
    \texttt{C\_block} is initialized as an empty \texttt{std::map} in \texttt{SpGeMM\_2d}, storing {\color{black} all entries}.

    \item \textbf{Output Conversion}:\\
        The \texttt{MTX2COO} function converts \texttt{C\_block} to COO format.\\
        {\color{black}It iterates over all entries in the sparse map, using their global coordinates directly to form the output matrix $C$.}
\end{enumerate}

% brief summary
{\color{black}For APSP, \texttt{SpGeMM\_2d} is used with the min-plus semiring, treating undeclared values in \texttt{C\_block} as infinity and setting diagonal entries to 0 to compute shortest paths correctly.}
...
\begin{itemize}[topsep=0pt, itemsep=0pt]
    \item \texttt{plus}:\\
          {\color{black}$\min(a, b)$, selecting the shorter path. If $a=0$ (infinity), returns $b$; otherwise, returns the minimum of $a$ and $b$.
    \item We treat all zero entries (assigned in the SpGeMM algorithm) in the resultant matrix as infinity. The error will occur on the diagonal line (they are true zeros), which will be replaced later.
          }
    \item \texttt{times}:\\
          {\color{black}$a + b$, summing edge weights directly, assuming valid weights (zeros as 0).}
\end{itemize}
{\color{black}This allows $L^k[i,j]$ to represent the shortest path from $i$ to $j$ using at most $k$ intermediate nodes. Diagonal entries are set to 0 after each iteration to ensure correctness.}
...
\begin{enumerate}
    ...
    \item \textbf{Local Processing}:\\
           {\color{black} Set diagonal entries $L[i,i]$ to 0 to correct potential errors from summation or minimization.}
...
\begin{itemize}[label={},itemsep=0pt,topsep=0pt] 
    ...
    \item \textbf{Comm. cost}:\\
        $\sqrt{p}$ \texttt{MPI\_Bcast} calls cost $O(\tau \cdot \sqrt{p} \cdot \log{p}+ \mu \cdot (\text{nnz}(A) 
        + \text{nnz}(B))\cdot \log{p}/\sqrt{p} 
        )$ per processor
\end{itemize}
...
\begin{itemize}[label={}, itemsep=0pt,topsep=0pt]   
    \item  \textbf{Comput.cost}:\\
        $O((\text{nnz}(G)^2 \cdot \log n)/{\sqrt{p}} + \text{nnz}(G) \cdot \log n)$ for $\text{nnz}(G)$ edges, due to $\lceil \log n \rceil$ \texttt{SpGeMM\_2d} calls and diagonal operation.   
    \item  \textbf{Comm. cost}:\\ 
        $\log n$ $\times$ SpGeMM’s cost $+$ \texttt{MPI\_Barrier}, totaling 
        		$O(
        		\tau \cdot \sqrt{p} \cdot \log n \log p 
        		+ \mu \cdot \text{nnz}(G) \cdot \log n \log p / \sqrt{p})$ per processor.
\end{itemize}
}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{\color{black}SpGeMM: 55.80s to 4.04s ($p=36$, speedup 13.80$\times$).}
\end{figure}

SpGeMM Scales from {\color{black}55.80s ($p=1$)} to {\color{black}4.04s ($p=36$)}, meeting $<10$s target. Speedup ({\color{black}13.80$\times$}) slows beyond $p=16$ due to communication ($O(\alpha \cdot \sqrt{p})$). 
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{\color{black}APSP: 869.63s to 11.01s ($p=36$, speedup 79.02$\times$).}
\end{figure}

APSP Drops from {\color{black}869.63s} to {\color{black}11.01s}, exceeding $<30$s target. Higher speedup ({\color{black}79.02$\times$}) reflects more enormous sequential workload, but $\log n$ iterations limit scalability.\\ 
