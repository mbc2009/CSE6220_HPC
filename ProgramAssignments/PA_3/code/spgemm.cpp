...
void SUMMA(int current_size_m, int current_size_p, int current_size_n,
    ...
    std::map<std::pair<int, int>, int> &C_block, int block_size_m, int block_size_n,
        ...
        for(int i=0; i<ni_A; i++){
            for(int j=tmp_A_row[i]; j<tmp_A_row[i+1];j++){
                ...
                for(int l=tmp_B_row[A_col]; l<tmp_B_row[A_col+1]; l++){
                    ...
                    //C_block[i][B_col] = plus(C_block[i][B_col], times(tmp_A_v[j], tmp_B_v[l]));
                }
            }
        }
    }   
        ...
        )
{
    ...
    for (auto &[key, val] : C_block) {
        int global_i = key.first + row_offset;
        int global_j = key.second + col_offset;
        if (val != 0) {
...
int current_size_m = std::max(0,std::min(m-(row_rank)*block_size_m, block_size_m));
int current_size_n = std::max(0,std::min(n-(col_rank)*block_size_n, block_size_n));
int current_size_p = std::max(0,std::min(p-(row_rank)*block_size_p, block_size_p));
